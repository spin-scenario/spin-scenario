...
void concurrent_block::evolution(int index) {
  allign();
  std::vector<timeline> keys;
  std::vector<timeline> terminals;
  for (size_t i = 0; i < sub_blocks_.size(); i++) {

    sub_blocks_[i]->switch2loop(index); // NOTE.
    std::vector<timeline> sub_keys = sub_blocks_[i]->absolute_keys();
  ...
  std::cout << "evolution concurrent-block " << index << " " << width_in_ms() << " ms.\n";
}

void concurrent_block::allign() {
  timeline t0 = sub_blocks_[0]->WIDTH();
  for (size_t i = 1; i < sub_blocks_.size(); i++) {
    std::string reg = sub_blocks_[i]->align_label();
    ...
    if (reg != "") {
      ...
      if (reg == "c")
        ...
        std::string s = "invalid align parameter: " + reg;
  ...
  std::vector<timeline> start_timeline;
  for (size_t i = 1; i < sub_blocks_.size(); i++)
...
std::vector<seq_block *> concurrent_block::simplify(const seq_block *sb) {
  std::vector<seq_block *> result;
  if (sb->category() == _glue) {
    glue *sb1 = (glue *) sb;
    if (sb1->style() == _concurrent) {
      std::vector<seq_block *> cur_sub_blocks = sb1->sub_blocks();
      for (size_t i = 0; i < cur_sub_blocks.size(); i++) {
        if (cur_sub_blocks[i]->category() != _glue)
          result.push_back(cur_sub_blocks[i]);
        else {
          std::vector<seq_block *> tmp = simplify(cur_sub_blocks[i]);
...
void concurrent_block::write(std::ostream &ostr) const {
  glue::write(ostr);
}
