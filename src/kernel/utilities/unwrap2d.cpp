...
// itself
void initialisePIXELs(float *wrapped_image, unsigned char *input_mask,
...
void gatherPIXELs(EDGE *edge, params_t *params) {
  ...
  for (k = 0; k < params->no_of_edges; k++) {
    ...
    // group no else or else if to this if
...
// phase map.
void returnImage(PIXELM *pixel, float *unwrapped_image, int image_width,
                 int image_height) {
  int i;
  int image_size = image_width * image_height;
